// This is an event like what is watched in a observer
class Event {
	constructor() {
	...
	// subscribe like an observer pattern
	subscribe(handler) {
	...
	// This is also like anotifier of an observer
	fire(sender, args) {
		this.handlers.forEach(function executeHandlersFunctions(fn, k) {
			fn(sender, args);
		});
	}
}
...
let doubleAttackModifier = new DoubleAttackModifier(game, goblin);
console.log(goblin.toString());

let increaseDefenseModifier = new IncreaseDefenseModifier(game, goblin);
console.log(goblin.toString());
increaseDefenseModifier.dispose();

doubleAttackModifier.dispose();
console.log(goblin.toString());
