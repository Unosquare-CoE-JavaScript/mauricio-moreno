...
// The data that is sended when person properties have changed
...
class RegistrationChecker {
	constructor(person) {
		...
		this.token = person.propertyChanged.subscribe(this.age_changed.bind(this)); // Bind ensures we use current this
	}

	// Takes the event and evaluates it
	age_changed(sender, args) {
		if (sender === this.person && args.name === 'age') {
			if (args.newValue < 13) console.log(`Sorry, you are still to young`);
			else {
