...
// vvv this is what you have to work with vvv
// This is simply a hashing function you dont need to understand it (if you want)
...
class LineToPointAdapter extends Array {
	constructor(line) {
		...
		let points = []; // here we can store some of the points added
		...
		LineToPointAdapter.cache[this.hash] = points; // The way we can access the data
	}

	get items() {
		return LineToPointAdapter.cache[this.hash]; // we can use the getter to get the cache
	}
}
LineToPointAdapter.count = 0;
LineToPointAdapter.cache = {}; // here we need to add the cache to the class externally
