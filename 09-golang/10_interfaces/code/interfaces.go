...
// Describer interface describes the struct beign passed in a function
...
func (g *Group) describe() string {
	desc := fmt.Sprintf(
		"The %s user group has %d users. Newest user:  %s, Accepting New Users:  %t",
		g.role,
		len(g.users),
		g.newestUser.FirstName,
		g.spaceAvailable,
	)
	return desc
}
...
// DoTheDescribing describes wathever struct is passed in
